...
|----------------------|--------------|------------------------------|---------------------------------------|
| unloadDelay          | `number`     | 10000                        | The time in milliseconds that all dialog components will remain mounted after being closed. This is used to ensure that any closing animations that the dialog needs to play have time to play out before it is removed from the virtual DOM.
...
This function returns a `Promise` that will resolve when the `submitDialog` callback is called from the `DialogComponent` with whatever value is passed to the `submitDialog` call. It will reject if the modal is closed without calling the `submitDialog` callback.
